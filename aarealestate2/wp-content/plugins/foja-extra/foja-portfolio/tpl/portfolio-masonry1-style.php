...
<div id="some-<?php echo esc_attr($rand_mason_porfo); ?>" class="portfolio-masonry-block porf-hidetitle-st clearfix">
	...
	<ul id="target-effect" class="grid effect-3 portfolio-block-wrap masonry-template<?php if($portfolio_pagination_type == 'portfolio_pagination_infinite') { ?> porto-masonry-wrap-<?php echo esc_attr($loop_infinite_class); } ?> column-<?php echo sanitize_text_field($choose_column); ?> clearfix">
		...
		<?php 
			...
			$foja_masonry = new WP_Query($foja_masonry_args);
			if ($foja_masonry->have_posts()) : while($foja_masonry->have_posts()) : $foja_masonry->the_post(); 
			...
			if(!empty($category_terms)){
				...
				$foja_porto_comas =  join( ", ", $category_name );
				$foja_porto_space =  join( " ", $category_slugs );

				}
			}
		?>

		<li class="portfo-block-item masonry-item <?php echo esc_attr($mobile_choose_column); ?> <?php echo esc_attr($tablet_choose_column); ?> <?php echo sanitize_text_field( $masonry_size_item ); ?> <?php echo sanitize_text_field($foja_porto_space); ?>">
			<div class="item-wrap">
					...
					<figure class="<?php echo sanitize_text_field( $hover_effect ); ?>">
						...
						<figcaption style="background-image: url(<?php echo esc_url($portfolio_alternative_image); ?>);">
							<div class="caption-inside">
								...
								<!-- if use category -->
								<h5 class="portfolio-category <?php echo sanitize_text_field ( $category_hover_effect ); ?> <?php echo sanitize_text_field ( $category_hover_delay ); ?>"><?php echo sanitize_text_field($foja_porto_comas); ?></h5>
								<!-- if use category -->
				...
				<div class="portfolio-details clearfix">
					...
					<div class="portfolio-excerpt">
						<p><?php echo foja_excerpt($excerpt_word); ?></p>
					</div>
					<?php }
	...
	if($pages == '')
	{
		$pages = $foja_masonry->max_num_pages;
		if(!$pages)
		{
			$pages = 1;
		}
		...
		<nav class="navigation-paging pagination pagination-page-template clearfix">
			<div class="container">
				<div class="post-navigation nav-previous pull-left">
					<?php next_posts_link( $pagination_prev_text, $foja_masonry->max_num_pages ); ?>
				</div>
	...
	<div class="container clearfix">
		...
		<?php } ?>
			<div id="load-more-loop-masonry-1-<?php echo esc_attr( $loop_infinite_class ); ?>" class="infinite-button">
				<?php next_posts_link( '', $foja_masonry->max_num_pages ); ?>
			</div>
