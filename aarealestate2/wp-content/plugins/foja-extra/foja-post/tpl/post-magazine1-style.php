...
<div class="post-list-1<?php if($use_infinite_scroll == 'use') { ?> list1-wrap-<?php echo esc_attr($loop_infinite_class); } ?> post-magazine-style-block clearfix">
	...
	<article class="post-item magazine-post-style magazine-1-post-style clearfix selector-padding selected-for-margin-bottom post-list-post-selector <?php echo sanitize_text_field($horizontal_col_select); ?> clearfix" data-file="<?php the_permalink(); ?>" data-target="article">
		...
		<div class="story-content">
				...
				<span class="author">
					<a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>">
						<span class="author-name">
							<?php foja_get_author_30(); ?>
							<span class="author-separator"><?php esc_html_e( 'by', 'foja' ); ?></span><span class="vcard"> <?php echo get_the_author_meta( 'display_name' ); ?></span>
						</span>
					</a>
				</span>
				<span class="date">
					<a href="<?php the_permalink(); ?>">
						<span><?php echo get_the_date('F'); ?></span> <span><?php echo get_the_date('d'); ?></span><?php esc_html_e( ',', 'foja' ); ?> <span><?php echo get_the_date('Y'); ?></span>
					</a>
				</span>
				<span class="time-read">
					<span class="eta"></span> <?php esc_html_e( ' read', 'foja' ); ?>
				</span>
			</div>
			<?php }
			...
			if($use_read_more == 'on') { ?>
				<a href="<?php the_permalink(); ?>" class="read-more"><?php esc_html_e( 'Continue Reading', 'foja' ); ?></a>
			<?php } ?>
		</div>
