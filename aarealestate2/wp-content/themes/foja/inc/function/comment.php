<?php if ( ! function_exists( 'foja_comment' ) ) :

function foja_comment( $comment, $args, $depth ) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
			...
			<div class="meta-comment">
                <div class="comment-author vcard">
                	<?php printf( esc_html__( '%s', 'foja' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>

                	<time datetime="<?php comment_time( 'c' ); ?>">
						<?php printf( _x( '%1$s', '1: date', 'foja' ), get_comment_date()); ?>
					</time>
                </div><!-- .comment-author .vcard -->
                
				<?php if ( $comment->comment_approved == '0' ) : ?>
                	<div class="waiting-moderation">
					<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'foja' ); ?></em>
					</div>
				<?php endif; ?>
				...
				<div class="comment-action">
	            	<span class="edit-comment-link"><?php edit_comment_link( esc_html__( 'Edit', 'foja' ), '<span class="edit-link">', '<span>' ); ?></span>
